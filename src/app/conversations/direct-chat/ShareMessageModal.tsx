...
import type { TStateDirectMessage, TConversationCard } from "@/utils/types/global"
import { EMessageTypes } from "@/utils/enums"

interface ShareMessageModalProps {
  open: boolean
  onClose: () => void
  messageToShare?: TStateDirectMessage
...
export const ShareMessageModal: React.FC<ShareMessageModalProps> = ({
  ...
  const [searchResults, setSearchResults] = useState<TConversationCard[]>([])
  ...
  // Lọc và giới hạn conversations - chỉ lấy direct chat
  const filteredConversations = useMemo(() => {
    // Nếu đang tìm kiếm, sử dụng kết quả từ API và lọc chỉ direct chat
    if (search.trim() !== "") {
    ...
    // Nếu không tìm kiếm, sử dụng conversations từ Redux (đã là direct chat)
    if (!allConversations) return []
    return allConversations.slice(0, CONVERSATIONS_LIMIT)
  }, [allConversations, search, searchResults])
      ...
      <div className="max-h-72 overflow-y-auto space-y-2 STYLE-styled-scrollbar pr-1">
        {isSearching ? (
          ...
          filteredConversations.map((conversation) => {
            const { id, avatar = null, title = "", subtitle = null, type = "DIRECT" } = conversation
            return (
              <button
                key={`${type}-${id}`}
                    ...
                    if (conversation.type === "DIRECT") {
                      try {
                        ...
                        if (messageToShare.type === EMessageTypes.STICKER) {
                          ...
                          payload = {
                            ...payload,
                            content: messageToShare.stickerUrl || "",
                          }
                          ...
                          payload = {
                            ...payload,
                            content: messageToShare.content || "",
                            mediaUrl: messageToShare.mediaUrl,
                            fileName: messageToShare.fileName,
                            fileType: messageToShare.fileType,
                          }
                        }
