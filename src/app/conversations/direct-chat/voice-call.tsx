...
const CallBox = ({ open, directChatId }: TCallBoxProps) => {
  const { startCall } = useVoiceCall()
  const user = useUser()!

  const voiceCallHandler = () => {
    startCall(user.id, directChatId, () => {})
  }
...
export const VoiceCall = ({ canSend, directChatId }: TVoiceCallProps) => {
  ...
  return (
    <div>
      ...
      <CallBox open={callBoxOpen} directChatId={directChatId} />
    </div>
  )
}
