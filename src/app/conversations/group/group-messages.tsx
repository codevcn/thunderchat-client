...
import type { TStateGroupMessage } from "@/utils/types/global"
...
const NoMessagesYet = ({ groupChat, user }: TNoMessagesYetProps) => {
...
export const GroupMessages = memo(({ groupChat }: TMessagesProps) => {
  const { id: groupChatId, creatorId, lastSentMessageId } = groupChat
  ...
  useEffect(() => {
    if (tempFlagUseEffectRef.current) {
      tempFlagUseEffectRef.current = false
      if (!messages || messages.length === 0) {
        fetchMessages(groupChatId, msgOffset.current, true)
      }
    }
      ...
      >
        {fetchedMsgs ? (
          messages && messages.length > 0 ? (
            ...
            <NoMessagesYet groupChat={groupChat} user={user} />
          )
        ) : (
