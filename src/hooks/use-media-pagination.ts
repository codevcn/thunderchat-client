...
import type { TMediaItem, TMediaFilters } from "@/utils/types/be-api"
            ...
            if ((filters as any).types && (filters as any).types.length > 0) {
              ...
              const allowedTypes = (filters as any).types
                .map((type: string) => {
                  switch (type) {
                    case "image":
                      return EMessageTypes.IMAGE
                    case "video":
                      return EMessageTypes.VIDEO
                    case "file":
                      return EMessageTypes.DOCUMENT
                    case "voice":
                      return EMessageTypes.AUDIO
                    default:
                      return null
                  }
                })
