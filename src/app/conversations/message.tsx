...
import { Check, CheckCheck, FileText, File, FileSpreadsheet, Presentation, FileCode } from "lucide-react"
...
const Content = ({ content, stickerUrl, mediaUrl, type, fileName, fileType, fileSize }: TContentProps) => {
   // Hiển thị ảnh
...
export const Message = ({ message, user, stickyTime }: TMessageProps) => {
   const { authorId, content, createdAt, isNewMsg, id, status, stickerUrl, mediaUrl, type, fileName, fileType, fileSize } = message
         ...
         <div className="w-full text-regular-white-cl">
            {user.id === authorId ? (
               <div className={`QUERY-user-message-${id} flex justify-end w-full`}>
                  ...
                  >
                     <Content content={content} stickerUrl={stickerUrl} mediaUrl={mediaUrl} type={type} fileName={fileName} fileType={fileType} fileSize={fileSize} />
