...
const Content = ({ content, stickerUrl, mediaUrl, type }: TContentProps) => {
   // Hiển thị ảnh
...
export const Message = ({ message, user, stickyTime }: TMessageProps) => {
   const { authorId, content, createdAt, isNewMsg, id, status, stickerUrl, mediaUrl, type } = message
         ...
         <div className="w-full text-regular-white-cl">
            {user.id === authorId ? (
               <div className={`QUERY-user-message-${id} flex justify-end w-full`}>
                  ...
                  >
                     <Content content={content} stickerUrl={stickerUrl} mediaUrl={mediaUrl} type={type} />
