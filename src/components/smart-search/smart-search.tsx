...
// --- TYPES ---
// Định nghĩa TMember khớp với dữ liệu thật từ InfoBar truyền xuống
export type TMember = {
  id: number
  email: string
  Profile: {
...
type SearchFilters = {
  startDate: string
  endDate: string
  authorId: string | null // Dùng string để dễ so sánh trong dropdown
}
...
export default function SmartSearch({ members, directChatId, onMessageClick }: SmartSearchProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSearching(true)
    setResults(null) // Clear kết quả cũ trước khi tìm mới
  ...
  const handleSelectUser = (userId: string | number | null) => {
    setFilters((prev) => ({ ...prev, authorId: userId?.toString() || null }))
    setShowUserDropdown(false)
  }

  // Lấy tên user đang chọn
  const selectedUserName =
    members.find((m) => m.id.toString() === filters.authorId)?.Profile.fullName || "All Users"
    ...
    <div className="w-full px-2 py-2">
      {!isExpanded ? (
        <div
          ...
          className="group flex items-center gap-3 px-3 py-2.5 bg-white/5 hover:bg-white/10 border border-transparent hover:border-purple-500/30 rounded-xl cursor-pointer transition-all duration-200"
        >
          <Search
            className="text-gray-400 group-hover:text-purple-400 transition-colors"
            size={18}
          />
          <span className="text-gray-500 text-sm font-medium group-hover:text-gray-300">
            Search or ask AI...
          </span>
        </div>
      ) : (
        <div className="bg-gray-900/95 border border-purple-500/30 rounded-xl shadow-2xl animate-in fade-in zoom-in-95 duration-150 overflow-visible">
          <div className="p-2">
            <form onSubmit={handleSubmit} className="flex items-center gap-2">
              ...
              <button
                ...
                className={`p-2 rounded-lg border transition-all ${showFilter ? "bg-purple-500/20 border-purple-500/50 text-purple-400" : "bg-gray-800 border-gray-700 text-gray-400 hover:text-white hover:bg-gray-700"}`}
          ...
          {showFilter && (
            <div className="px-3 pb-3 pt-1 border-t border-white/5 bg-black/20 animate-in slide-in-from-top-2 duration-200 relative z-10">
              <div className="space-y-3">
                ...
                <div className="relative" ref={userDropdownRef}>
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-between bg-gray-800 border border-gray-700 rounded-md px-3 py-2 text-xs text-white hover:border-purple-500/50 focus:outline-none transition-colors"
                  >
                  ...
                  {showUserDropdown && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-gray-800 border border-gray-700 rounded-md shadow-xl max-h-48 overflow-y-auto z-50 STYLE-styled-scrollbar">
                      <div
                        onClick={() => handleSelectUser(null)}
                        className="flex items-center justify-between px-3 py-2 hover:bg-white/10 cursor-pointer text-xs text-white border-b border-white/5"
                      >
                        <span>All Users</span>
                      ...
                      {members.map((member) => (
                        <div
                          ...
                          className="flex items-center justify-between px-3 py-2 hover:bg-white/10 cursor-pointer text-xs text-white"
                        >
                          <div className="flex items-center gap-2 truncate">
                            <div className="w-5 h-5 rounded-full bg-gray-600 flex-shrink-0 overflow-hidden flex items-center justify-center">
                          ...
                          {filters.authorId === member.id.toString() && (
          ...
          {results && !isSearching && (
            <div className="px-3 pb-3 max-h-[60vh] overflow-y-auto STYLE-styled-scrollbar border-t border-white/10 pt-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
              {/* 1. AI Answer Card */}
              {results.answer && (
                  ...
                  <div className="flex flex-col gap-2">
                    {results.sources.map((source) => (
                      <div
                        ...
                        onClick={() => onMessageClick?.(source.messageId)}
